...
com.google.android.gms.location.LocationListener {
	...
	private String id_alimentoBuscar;
	private Bundle mbundle;
	
	
	public static FragmentMapa NewInstance(Bundle arguments){
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(mbundle != null){
			id_alimentoBuscar = mbundle.getString("id");
			Toast.makeText(getActivity(), id_alimentoBuscar, Toast.LENGTH_SHORT).show();
		}
		if(services0K()){
	...
	public void onLocationChanged(Location location) {
		...
		if(id_alimentoBuscar == null){
			iniciarMarkerRestaurant();
			iniciarMarkerLocal();
		}
		else{
			ib_local.setVisibility(View.GONE);
			ib_restaurant.setVisibility(View.GONE);
			iniciarMarkerLocalconIngrediente();
		}
	...
	private void iniciarMarkerLocalconIngrediente() {
    	...
    	query.findInBackground(new FindCallback<ParseObject>() {
			...
			@Override
			public void done(List<ParseObject> lista_locales, ParseException e) {
				for(final ParseObject object : lista_locales){
					ParseQuery<ParseObject> queryAlimento = ParseQuery.getQuery("AlimentoLocal");
					queryAlimento.whereEqualTo("local", object);
					queryAlimento.include("alimento");
					queryAlimento.findInBackground(new FindCallback<ParseObject>() {
						
						@Override
						public void done(List<ParseObject> lista_alimentos, ParseException e) {
							if(e==null){
								for(ParseObject alimentos : lista_alimentos){
									ParseObject alimento = alimentos.getParseObject("alimento");
									if(alimento.getObjectId().equals(id_alimentoBuscar)){
										Toast.makeText(getActivity(), "Encontraste el producto en : " + object.getString("nombre"), Toast.LENGTH_SHORT).show();
										LatLng position = new LatLng(object.getParseGeoPoint("location").getLatitude(), object.getParseGeoPoint("location").getLongitude());
										...
										hashLocal.put(marker, object.getObjectId());
										hashNombreLocal.put(marker, object.getString("nombre"));
										break;
									}
								}
							}
	...
	protected void mostrarDialogLocal() {
		...
		dialogLocal.show();
		mLinearListView = (LinearLayout) dialogLocal.findViewById(R.id.linear_listview);
	...
	protected void mostrarDialogAlimento() {
		...
		AdapterAlimentos adapter = new AdapterAlimentos(getActivity(), mLinearListView, dialogAlimento);
